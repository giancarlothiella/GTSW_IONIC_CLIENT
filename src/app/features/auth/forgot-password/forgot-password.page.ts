        ...
        <ion-card class="forgot-password-card">
          ...
          <ion-card-content>
            ...
            <div class="input-section">
              <ion-text color="dark">
                <p class="input-label">{{ getText(202) }}</p>
              </ion-text>
              <ion-input
                type="email"
                inputmode="email"
                [(ngModel)]="formData.email"
                [placeholder]="getText(203)"
                class="custom-input"
                [disabled]="isLoading">
              </ion-input>
            </div>
            ...
            <div class="button-container">
              <ion-button
                ...
                [disabled]="isLoading"
                class="submit-button">
                {{ isLoading ? getText(204) : getText(205) }}
              </ion-button>
            </div>
...
export class ForgotPasswordPage implements OnInit {
  ...
  async onSubmit() {
    if (!this.formData.email) {
      this.errorMessage = this.getText(207);
      return;
    }
    ...
    const loading = await this.loadingCtrl.create({
      message: this.getText(204)
    });
    await loading.present();

    try {
      const result = await this.authService.resetPassword(this.formData.email);
      ...
      if (result && result.valid) {
        ...
        this.errorMessage = result.message || this.getText(209);
      ...
      await loading.dismiss();
      this.isLoading = false;
      this.errorMessage = error.message || this.getText(209);
