...
import { GtsAnalyzerSetupDialogComponent } from '../../../core/gts-open-source/gts-analyzer-setup-dialog/gts-analyzer-setup-dialog.component';
...
@Component({
  ...
  imports: [
    ...
    GtsMessageComponent,
    GtsLoaderComponent,
    GtsAnalyzerSetupDialogComponent
  ],
  templateUrl: './ai-analyzer.page.html',
  styleUrls: ['./ai-analyzer.page.scss']
})
export class GTSW_AiAnalyzerComponent implements OnInit, OnDestroy {
  //========= PAGE PARAMS =================
  prjId: string = 'GTSW';
  formId: number = 15;
  ...
  showAnalyzerDialog: boolean = false;
  analyzerMode: 'new' | 'edit' = 'new';
  analyzerEditingCode: string | null = null;

  //========= PAGE FUNCTIONS =================
  async getCustomData(prjId: string, formId: number, customCode: string, actualView: string) {
    //===== START CUSTOM CODE =====

    if (customCode === 'OPEN_ANALYZER_SETUP_NEW') {
      this.analyzerMode = 'new';
      this.analyzerEditingCode = null;
      this.showAnalyzerDialog = true;
    }

    if (customCode === 'OPEN_ANALYZER_SETUP_EDIT') {
      const row = this.gtsDataService.getDataSetSelectRow(this.prjId, this.formId, 'daAnalyzers', 'qAnalyzers');
      if (row?.code) {
        this.analyzerMode = 'edit';
        this.analyzerEditingCode = row.code;
        this.showAnalyzerDialog = true;
      }
    }
  ...
  onAnalyzerSaved(_ev: { code: string }): void {
    this.gtsDataService.sendGridReload('qAnalyzers');
  }
