...
export class GtsGridService {
  ...
  async getGridData(prjId: string, formId: number, metaData: any, pageData: any, getCustomVerifyResult: any) {
    ...
    // Usa sempre ArrayStore (comportamento attuale)
    // La paginazione lato server viene gestita limitando i dati iniziali dal server
    data.dataStore = new ArrayStore({
      data: dataSet,
      key: keys
    });
    ...
    if (data.dataFilter !== undefined && data.dataFilter !== null && data.dataFilter.length > 0) {
      data.dataSource.filter(data.dataFilter);
      data.filtered = true;
    }
